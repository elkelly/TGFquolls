# targeted gene flow on quoll population model with Outbreeding Depression
...
managSpaceOBRR <- function(alpha, recb, Nstar=500, h2=0.2, W0=0.05, props=seq(0, 0.3, 0.02), yr=seq(from=21,to=41,by=1), n=100) {
  #input
  nI<-floor(Nstar*props) # number to introduce
  fName<-paste0("../out/quollNewOB", round(alpha*100, digits = 0), "RR", recb*100, ".RData") # filename to save to
  ...
  for(k in yr) {
    ...
    for(j in nI) {
      ...
      for (i in 1:n) {
        ...
        test<-TGF(demPars = pars, nP = 10, nN = 10, nI = 10, W0 = 0.05, f0 = 0.05, s0=0.38, K = Nstar, h2=h2, burnIn=10, noSel=20, selGens=50, TGFtime=k, f1=0.9, nIntr=j, outbreeding=TRUE, nl=10, H2=1, alpha=alpha, recombRate=recb)    
