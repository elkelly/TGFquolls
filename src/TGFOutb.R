...
for(k in yr) {
  ...
  for(j in nI) {
    for (i in 1:n) {
      test<-TGF(demPars = pars, nP = 10, nN = 10, nI= 10, W0 = 0.09, s0=0.38, h2=0.2, burnIn=10, noSel=20, selGens=50, TGFtime=k, W1=0.9, nIntr=j, outbreeding=TRUE, nl=10, H2=1, alpha=alpha50)    
...
for(k in yr) {
  ...
  for(j in nI) {
    for (i in 1:n) {
      test<-TGF(demPars = pars, nP = 10, nN = 10, nI= 10, W0 = 0.09, s0=0.38, h2=0.2, burnIn=10, noSel=20, selGens=50, TGFtime=k, W1=0.9, nIntr=j, outbreeding=TRUE, nl=10, H2=1, alpha=alpha10)    
      e[i]<-ifelse(tail(test$n,1)<=1, TRUE, FALSE)
      g[i]<-tail(test$genome[,1], 1)      
      g2[i]<-tail(test$genome[,2], 1)
      Ip[i]<-ifelse(is.null(test$IntProp), NA, test$IntProp)
